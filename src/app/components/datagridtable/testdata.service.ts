  ...
  handleDataChange() {
    ...
    this.datagridTableService.stateChange_.pipe(
      tap(() => this.datagridTableService.setShowMessageOverlay(false)),
        filter((event) => event === EDatagridTableStateChangeEvents.CHANGE_PAGE || event === EDatagridTableStateChangeEvents.CHANGE_SORT || event === EDatagridTableStateChangeEvents.CHANGE_DATA),
        startWith({}),
        switchMap(() => {
